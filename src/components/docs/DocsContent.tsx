      ...
      for (const cp of codeParts) {
        if (cp.startsWith("`") && cp.endsWith("`")) {
          nodes.push(
            <code
              key={nodes.length}
              className="rounded bg-white/[0.06] px-1.5 py-0.5 font-mono text-[0.75rem] text-amber-300/90"
            >
              {cp.slice(1, -1)}
            </code>
          );
...
export default function DocsContent({ markdown }: DocsContentProps) {
  ...
  while (i < lines.length) {
    ...
    if (trimmed.startsWith("# ") && !trimmed.startsWith("## ")) {
      elements.push(
        <h1 key={`h1-${i}`} className="font-clash text-2xl font-bold text-white mb-4 mt-2">
    ...
    if (trimmed.startsWith("## ") && !trimmed.startsWith("### ")) {
      elements.push(
        <h2 key={`h2-${i}`} className="font-clash text-lg font-semibold text-white/90 mb-3 mt-6">
    ...
    if (trimmed.startsWith("### ")) {
      elements.push(
        <h3 key={`h3-${i}`} className="font-clash text-base font-semibold text-white/80 mb-2 mt-4">
    ...
    if (trimmed.startsWith("- ") || trimmed.startsWith("* ")) {
      ...
      elements.push(
        <ul key={`ul-${i}`} className="list-disc pl-5 my-2 space-y-1 text-sm text-white/70">
    ...
    if (/^\d+[.)]\s/.test(trimmed)) {
      ...
      elements.push(
        <ol key={`ol-${i}`} className="list-decimal pl-5 my-2 space-y-1 text-sm text-white/70">
          {listItems.map((item, idx) => (
            <li key={idx}>{renderInline(item)}</li>
          ))}
        </ol>
    ...
    if (trimmed.startsWith("> ")) {
      ...
      elements.push(
        <blockquote key={`quote-${i}`} className="border-l-2 border-amber-500/30 pl-4 my-3 text-sm text-white/60 italic">
    ...
    // Default: paragraph
    elements.push(
      <p key={`p-${i}`} className="text-sm text-white/70 leading-relaxed mb-3">
...
function renderTable(rows: string[], key: string): React.ReactNode {
  ...
  return (
    <table key={key} className="w-full text-sm border-collapse">
        ...
        <thead>
          <tr>
            {headerCells.map((cell, ci) => (
              <th
                key={ci}
                className="border border-white/[0.08] bg-white/[0.04] px-3 py-2 text-left font-semibold text-white/80 text-xs uppercase tracking-wider"
              >
                {renderInline(cell)}
              </th>
            ))}
          ...
          {bodyRows.map((row, ri) => (
            <tr key={ri}>
              {parseRow(row).map((cell, ci) => (
                <td
                  key={ci}
                  className="border border-white/[0.06] px-3 py-2 text-white/70"
                >
                  {renderInline(cell)}
                </td>
              ))}
